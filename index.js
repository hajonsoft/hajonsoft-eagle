...
async function runGetSMSNumber() {
  ...
  inquirer.prompt({
    ...
    if (answers.api_key) {
      ...
      let balance = '';
      const balanceInquiry = await axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=getBalance`);
      ...
      if (activation) {
        const activationInquiry = axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=getStatus&id=${activation}`);
        ...
        inquirer.prompt([{
          ...
          if (answers.activationAction) {
            if (answers.activationAction.startsWith('1-')) {
              const cancelInquiry = await axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=setStatus&status=8&id=${activationId}`)
            ...
            if (answers.activationAction.startsWith('2-')) {
              const codeInquiry = await axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=setStatus&status=1&id=${activationId}`)
            ...
            if (answers.activationAction.startsWith('3-')) {
              const activationId = activation.split(":")[1]
              const statusInquiry = await axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=getStatus&id=${activationId}`)
              if (statusInquiry.status === 200) {
                ...
                if (statusInquiry.data.startsWith('STATUS_OK')) {
                  ...
                  const completeInquiry = await axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=setStatus&status=6&id=${activationId}`)
      ...
      const countriesInquiry = await axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=getCountries`);
      if (countriesInquiry.status === 200) {
        inquirer.prompt([{
          type: 'list',
          name: 'country',
          message: `change country: ${country}`,
          choices: [`${country}`, ...Object.values(countriesInquiry.data).filter(country => country.visible).map(country => `${country.id}:${country.eng}`)]
        }]).then(async answers => {
          console.log(answers.country)
          country = answers.country.split(':')[0] | '0'; // russia by default
          return fs.writeFileSync('./country', answers.country.split(':')[0]);
        })
      }

      const numberInquiry = await axios.get(`https://api.sms-activate.org/stubs/handler_api.php?api_key=${api_key}&action=getNumber&service=ot&country=${country}&freePrice=true&maxPrice=1`)

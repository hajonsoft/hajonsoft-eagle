...
async function sendToCloud(data) {
  ...
  const command = `git add . && git commit -m "${data.system?.country?.code} ${data.travellers?.length} Pax ${data.system?.name} ${data.system?.username} ${process.argv.find(arg => arg.toLowerCase().startsWith("range"))}.0${moment().format("mm")}" && git push origin $(git branch --show-current):job --force`;

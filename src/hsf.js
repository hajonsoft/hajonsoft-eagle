        ...
        page,
        {
          controller: {
            ...
            twfAction: async () => {
              ...
              // Edit to use correct hijri year in address
              await twfPage.goto(
                "https://www.etawaf.com/tawaf43/index.html?locale=en",
                {
                  waitUntil: "domcontentloaded",
                }
              );
...
async function onTWFPageLoad(res) {
  const mofaUrl = await twfPage.url();
  if (!mofaUrl) {
    return;
  }

  if (mofaUrl.toLowerCase().includes("_frmlogin.aspx".toLowerCase())) {
    await twfPage.waitForSelector("#txtUserName");
    await twfPage.type("#txtUserName", data.system.username);
    await twfPage.type("#txtPwd", data.system.password);
    return;
  }
  if (
    mofaUrl.toLowerCase().includes("Waytoumrah".toLowerCase()) &&
...
async function injectTWFEagleButton() {
  return;
  try {
    const tdSelector =
      "#Table1 > tbody > tr:nth-child(1) > td > table > tbody > tr:nth-child(3) > td:nth-child(2)";

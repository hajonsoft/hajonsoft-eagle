...
async function injectNSKEagleButton() {
  try {
    const tdSelector =
...
async function onNSKPageLoad(res) {
  ...
  if (mofaUrl.toLowerCase().includes("bsp-nusuk.haj.gov.sa/Identity".toLowerCase())) {
  ...
  // OTP
  if (mofaUrl.toLowerCase().includes("haj.gov.sa/OTP/GoogleAuth".toLowerCase())) {
  ...
  if (mofaUrl.toLowerCase().includes("nusuk.haj.gov.sa/ExternalAgencies/Groups/ViewPackage/".toLowerCase())) {
    await injectNSKEagleButton();
  }
}

async function handleImportNSKMofa() {
  console.log({ data });
  const tableSelector = "#kt_content > div > div > div > div.kt-portlet__body.px-0- > div:nth-child(1) > div > div > div > div > div.kt-widget__body > table";
  const table = await page.$(tableSelector);
  if (!table) {
    return;
  }

  const tableRows = await page.$$("#kt_content > div > div > div > div.kt-portlet__body.px-0- > div:nth-child(1) > div > div > div > div > div.kt-widget__body > table > tbody > tr");
  const passports = [];
...
module.exports = { initialize, injectNSKEagleButton, onNSKPageLoad };

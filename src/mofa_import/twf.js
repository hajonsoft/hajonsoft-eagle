  ...
  try {
    ...
    await page.$eval(
      ...
      (el) =>
        ...
        <div>HAJOnSoft</div>
        <button style="color: white; background-color: forestgreen; border-radius: 16px; padding: 16px;" type="button"  onclick="handleImportTWFMofa(); return false">To Eagle (current view) النسر </button>
        </div>
        `)
    );
  } catch (err) {
...
async function onTWFPageLoad(res) {
  ...
  const currentConfig = util.findConfig(mofaUrl, config);

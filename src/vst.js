  ...
  switch (currentConfig.name) {
    ...
    case "index":
      ...
      await page.type(
        "#txtGroupName",
        (
          data[counter].name.full +
          data[counter].nationality.name +
          data[counter].mobileNumber
        ).replace(/ /g, "")
      );
...
async function getEmailAddress() {
  ...
  await emailPage.goto("https://www.minuteinbox.com/", {
...
async function activateAccount() {
  ...
  try {
    for (let i = 0; i < 10; i++) {
      ...
      const response = await emailPage.goto(
        "https://www.minuteinbox.com/email/id/2",
        { waitUntil: "domcontentloaded" }
      );
...
async function receiveActivationCode() {
  ...
  try {
    for (let i = 0; i < 10; i++) {
      await emailPage.waitForTimeout(3000);
      const response = await emailPage.goto(
        "https://www.minuteinbox.com/email/id/3",
        { waitUntil: "domcontentloaded" }
      );
